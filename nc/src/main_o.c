...
volatile bool is_available[EQNUM] = {false, false, false};
...
static void print_term(const char *s)
{
	attron(A_UNDERLINE);
	move(LINES - 5, 0);
	clrtoeol();
...
static void print_term_eq(int i, const char *s)
{
	attron(COLOR_PAIR(4));
	move(LINES - 4 + i, 0);
	clrtoeol();
